...
- Finding handoffs must use Baton protocol 6. Before publishing or consuming a handoff, locate the locally deployed Baton distribution and read its `AGENTS-MAILBOX-PROTO.md` in full. The deployment supplies the executable and explicit absolute config path; never infer or hard-code either.
