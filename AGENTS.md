...
- Finding handoffs must use the protocol declared by the locally deployed Baton instance. Before publishing or consuming a handoff, locate the locally deployed Baton distribution and read its `AGENTS-MAILBOX-PROTO.md` in full. The deployment supplies the executable and explicit absolute config path; never infer or hard-code either.
- This project's Baton identities are `workflows.reviewer` for the reviewer and `workflows.implementer` for the implementer. Resolve role-only instructions to those identities; do not substitute a participant from another domain.
...
- Represent finding-folder material in Baton handoffs with `--references`, never a hash-pinned `--attach`. An external part pins `ROOT:path` and becomes damaged if the bytes change or the path disappears; a message-specific claim then fails closed, while a bare claim skips the damage and can consume later healthy deliveries. Apply the same restraint to every document still under revision. Use `--attach` only when both the bytes and the path are immutable for the message's required lifetime — that is a guarantee the sender must be able to establish, not a property any file class has by default.
- Finding folders are **ephemeral**, but their record must reach Git before cleanup: every finding folder is committed and pushed at least once, even when it is included only "for the record" or is unrelated to the commit's main change. After the branch merges to main, the resolution is closed, and the record-bearing commit is pushed, delete the closed folder in a follow-up cleanup commit and push that commit. No permanent or residual reference may point at the deleted folder — not from code comments, source, tests, runners, or tools. Anything a finding produces that must outlive the branch (regression tests, doc updates, refactor-trigger entries) lands in the tree proper before the folder is closed, phrased so it stands alone without the folder.
