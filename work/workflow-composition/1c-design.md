...
| `resolved_exception(6)` | an authorized (human) resolution disposed of the child's own blocked checkpoint. From the parent's view this is **indistinguishable from `reversed(5)`** — settle identically. The parent never learns *how* the child's unwind concluded, matching "the parent must never enumerate... the child's internal compensations." |
