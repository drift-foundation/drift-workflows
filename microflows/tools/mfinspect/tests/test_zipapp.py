...
Covers the contract a consumer relies on: a single `./mfinspect` file that runs on
Python 3.10+ with no virtualenv and no package install — reporting the right
version, showing help, bundling PyMySQL, and rebuilding cleanly.

Modeled directly on ../../../../mariachi/tests/test_zipapp.py.
"""
import hashlib
import os
import re
...
COMMITTED_ARTIFACT = REPO_ROOT / "mfinspect"
...
class ZipappBuildTests(unittest.TestCase):
	...
	def setUpClass(cls) -> None:
		cls._tmp = tempfile.TemporaryDirectory(prefix="mfinspect-zipapp-test-")
		cls.tmp_path = Path(cls._tmp.name)
		cls.artifact = cls.tmp_path / "mfinspect"
	...
	def test_version_reports_pyproject_version(self) -> None:
		...
		self.assertEqual(result.stdout.strip(), f"mfinspect {self.version}")

	def test_help_runs_standalone(self) -> None:
		result = self._run_artifact("--help")
		self.assertEqual(result.returncode, 0, result.stderr)
		self.assertIn("usage: mfinspect", result.stdout)
	...
	def test_pymysql_is_embedded_and_importable(self) -> None:
		...
		self.assertIn(f"mfinspect{os.sep}pymysql", probe.stdout)

	def test_metadata_is_bundled(self) -> None:
		names = zipfile.ZipFile(self.artifact).namelist()
		self.assertIn(f"mfinspect-{self.version}.dist-info/METADATA", names)
	...
	def test_clean_rebuild_replaces_stale_target(self) -> None:
		...
		self.assertEqual(check.stdout.strip(), f"mfinspect {self.version}")
	...
	def test_failed_build_without_existing_artifact_creates_none(self) -> None:
		...
		leftovers = [p.name for p in out.parent.iterdir() if p.name.startswith(".mfinspect-zipapp-")]
...
class CommittedArtifactTests(unittest.TestCase):
	"""Guards against the committed ./mfinspect drifting from source. Because builds
	are reproducible, a stale binary is caught by a byte-for-byte hash comparison."""

	def test_committed_artifact_exists_and_is_executable(self) -> None:
		self.assertTrue(
			COMMITTED_ARTIFACT.is_file(),
			"no committed ./mfinspect — run `just setup && just build` and commit it",
		)
	...
	def test_committed_artifact_matches_fresh_build(self) -> None:
		if not COMMITTED_ARTIFACT.is_file():
			self.skipTest("no committed ./mfinspect artifact")
		with tempfile.TemporaryDirectory(prefix="mfinspect-committed-") as tmp:
			fresh = Path(tmp) / "mfinspect"
			result = _build(fresh)
			self.assertEqual(result.returncode, 0, f"{result.stdout}\n{result.stderr}")
			self.assertEqual(
				_sha256(COMMITTED_ARTIFACT),
				_sha256(fresh),
				"committed ./mfinspect is stale — rebuild with `just setup && just build` "
