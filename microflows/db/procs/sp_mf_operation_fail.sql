DELIMITER $$
-- Durable NON-RETRYABLE operation failure (microflows_design.md §3.1). A
-- definite participant rejection (e.g. invalid request / identity conflict) is
-- not retryable: the workflow must stop advancing forward. This atomically
-- transitions forward(1) -> blocked_resolution(3) with disposition failed(2),
-- clears the lease, and records the failure in the audit event — so the
-- workflow is no longer claimable on the forward path and cannot repeat
-- indefinitely after lease expiry.
--
-- Fenced + identify-first + time-disciplined, like settle. Idempotent: if the
-- workflow is already blocked_resolution, returns 'already_blocked'.
CREATE PROCEDURE `sp_mf_operation_fail`(
...
proc:BEGIN
	...
	-- Idempotent: already blocked (lease-independent, like settle's terminal read).
	IF v_state = 3 THEN
