...
proc:BEGIN
	...
	IF v_existing_status = CONST_STATUS_WORKING THEN
		...
		ELSE
			-- 0.5 strict monotonicity: caller's settle time MUST be strictly after the last recorded
			-- event (equal disallowed) -> else EventTimeConflict (errno 30002). Caller's time verbatim.
